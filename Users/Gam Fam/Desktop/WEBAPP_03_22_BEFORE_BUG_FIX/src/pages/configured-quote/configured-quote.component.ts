...
export class ConfiguredQuoteComponent implements OnInit {
    ...
    // ngDoCheck() {
    //     console.log('somehting just changed=***=>>>', this.quoteData)
    // }
    ...
    onLoad() {
        ...
        if (this.commonService.getCreateQuoteVo().draftPath == '/configuredQuote' && this.commonService.getCreateQuoteVo().isEditDraftQuote) {
            console.log('iff0   getQuote Data===>>>', this.commonService.getCreateQuoteVo().isEditDraftQuote)
            if (this.commonService.getCreateQuoteVo().isEditDraftQuote) {
                ...
                let getQuoteData = this.commonService.getCreateQuoteVo();
                console.log('if0   getQuote Data===>>>', getQuoteData)
            ...
            console.log('elseee0   getQuote Data===>>>', this.commonService.getCreateQuoteVo().isEditSavedQuote)
            if (this.commonService.getCreateQuoteVo().isEditSavedQuote) {
                ...
                this.locstr.setObj('disclaimerAndNotes', this.disclaimerAndNotes);
                let getQuoteData = this.commonService.getCreateQuoteVo();
                console.log('else0   getQuote Data===>>>', getQuoteData)
                getQuoteData.parts.quoteNo = getQuoteData.quoteNo;
                this.locstr.setObj('parts', getQuoteData.parts);
                this.saveDraftQuote = true;
                if (this.locstr.get('fromViewQuoteOnlyView')) {
        ...
        if (this.isCreateNewAccessory) {
            ...
            console.log('if   config data===>>>', this.configureData)
        } else {
            this.configureData = this.quoteService.configureQuoteVoFormation();
            console.log('else  config data===>>>', this.configureData)
        }
 ...
 **/
    ...
    ngAfterViewInit() {
        ...
        } else if (activeSeries == "MVEC") {
            this.controllerSeries = 'micro_vision';
        }
    ...
    public createPanelDiagram = () => {
        ...
        for (let i = 0; i < optionLength; i++) {
            ...
            if (this.finalQuote.selectedPanelOptions[i].tag == 'IV') {
    ...
    public searchObj = (obj, query) => {
        for (var key in obj) {
            ...
            if (key === query) {
                ...
                if (value == 'PH-SEN' && this.selectedControllerSeries.tag == 'WCT600P') {

                    if (obj.isChecked) {
                        this.panelComponents.ph = true;
                        this.panelComponents.conductivity = true;
                        console.log('WHEREIS###600P1##--------------------', obj.isChecked);
                    }

                }

                if (value == 'PH-SEN' && this.selectedControllerSeries.tag == 'WCT600H') {

                    if (obj.isChecked) {
                        this.panelComponents.ph = true;
                        this.panelComponents.conductivity = true;
                        console.log('WHEREIS###600H1##--------------------', obj.isChecked);
                    }

                }

                if (value == 'ORP-SEN' && this.selectedControllerSeries.tag == 'WCT600P') {

                    if (obj.isChecked) {
                        this.panelComponents.orp = true;
                        this.panelComponents.conductivity = true;
                        //console.log('WHEREIS###600P2#--------------------',obj.isChecked);
                    }

                }


                if (value == 'ORP-SEN' && this.selectedControllerSeries.tag == 'WCT600H') {

                    if (obj.isChecked) {
                        this.panelComponents.ph = true;
                        this.panelComponents.conductivity = true;
                        //console.log('WHEREIS###600H2#--------------------',obj.isChecked);
                    }

                }


                if (value == 'ORPF-1' && this.selectedControllerSeries.tag == 'WCT600P') {
                ...
                if (value == 'ORPF-1' && this.selectedControllerSeries.tag == 'WCT600H') {
    ...
    discountBasedOnNoofSets(part, noOfSets) {
        ...
        if (this.quoteData && this.quoteData.parts && this.quoteData.parts.length > 0) {
            for (let i = 0; i < this.quoteData.parts.length; i++) {
                ...
                if (this.quoteData.QuotationType.type === 'q1' ||  this.quoteData.QuotationType.type == 'q4' || this.quoteData.QuotationType.type == 'q5') {
                    if (this.quoteData.parts[i] && (this.quoteData.parts[i].isController || this.quoteData.parts[i].isSensor || this.quoteData.parts[i].isPanelController)) {
