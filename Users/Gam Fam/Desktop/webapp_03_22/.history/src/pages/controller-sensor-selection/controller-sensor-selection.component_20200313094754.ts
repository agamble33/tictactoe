...
<<<<<<< HEAD
    ...
    constructor(public constants: Constants, private locstr: LocalStorageService,
        public sidebar: SidebarService, private toastService: ToastService,
        private commonService: CommonService, private broadcaster: Broadcaster,
        private userService: UserService, private router: Router,
        private quoteService: QuoteService, private cdr: ChangeDetectorRef) {
=======

    constructor(public constants: Constants, private locstr: LocalStorageService, public sidebar: SidebarService, private toastService: ToastService,
        private commonService: CommonService, private broadcaster: Broadcaster, private userService: UserService, private router: Router, private quoteService: QuoteService, private cdr: ChangeDetectorRef) {
>>>>>>> 309b392c344eeefc42edd5eeea765ffec4ade513
    }
    ...
    ngOnInit() {
       ...
       // this.locstr.set('saveQuoteFlag', false);
        ...
        if (this.selectedControllerSeries.tag == 'WCT900P' || this.selectedControllerSeries.tag == 'WCT900H' ||
        this.selectedControllerSeries.tag == 'WCT910H' || this.selectedControllerSeries.tag == 'WCT910P' ||
        this.selectedControllerSeries.tag == 'WCT930P' || this.selectedControllerSeries.tag == 'WCT930H') {
        ...
        if (this.commonService.getCreateQuoteVo().draftPath == '/controllerSensorSelection' &&
        ...
        if (!this.commonService.isCreateQuoteRoute(nextState['url']) && !this.isSaveDraftBtnClicked) {
            if (nextState['url'] === "/") {
                ...
                return Observable.create((observer: Observer<boolean>) => {
                  if( (this.locstr.get('isEditViewedQuote') === 'false') && (this.locstr.get('fromViewQuoteOnlyView') === 'false' &&
    ...
    protected getControllerSensorData = () => {
        ...
        this.userService.getControllerSensorIO(this.selectedControllerSeries.id).subscribe(
            res => {
                if (res.status == "success") {
                    this.seriesList = this.userService.getFormattedSensorList(res.data);

                    // show selected IO options if present in local storage
                    if (this.commonService.getCreateQuoteVo() && this.commonService.getCreateQuoteVo().selectedSensorList) {
                        let selectedSensorList: any = this.commonService.getCreateQuoteVo().selectedSensorList;
                        if ((this.seriesList && this.seriesList.length > 0) && (selectedSensorList && selectedSensorList.length > 0)) {
                            for (let i = 0; i < selectedSensorList.length; i++) {
                                for (let j = 0; j < this.seriesList.length; j++) {
                                    if (selectedSensorList[i].id === this.seriesList[j].id) {
                                        this.setSensorSelection(this.seriesList[j], selectedSensorList[i]);
                                        break;
                                    }
                                }
                            }
                    ...
                    this.seriesList = this.userService.getFormattedSensorList(this.seriesList);
                    ...
                    if (this.selectedControllerSeries.tag == 'PA-2') {
                        for (let k = 0; k < this.seriesList.length; k++) {
                            if (this.seriesList[k].tag == 'SO' && this.seriesList[k].childs && this.seriesList[k].childs.length) {
                                this.seriesList[k].isChecked = true;
                                for (let m = 0; m < this.seriesList[k].childs.length; m++) {
                                    if (this.seriesList[k].childs[m].tag == 'WAL-SEN' && this.seriesList[k].childs[m].childs &&
                                    this.seriesList[k].childs[m].childs.length) {
                                        this.seriesList[k].childs[m].isChecked = true;
                                        for (let n = 0; n < this.seriesList[k].childs[m].childs.length; n++) {
                                            if (this.seriesList[k].childs[m].childs[n].tag == 'CON-SEN') {
    ...
    setSensorSelection(seriesItem: any, localSavedItem: any) {
        ...
        if ((seriesItem.tag == 'AN-PTSA-SEN' && seriesItem.childs.length > 0) ||
                ...
                for (let i = 0; i < seriesItem.childs.length; i++) {
                    for (let j = 0; j < localSavedItem.childs.length; j++) {
                        if (seriesItem.childs[i].id == localSavedItem.childs[j].id) {
                            this.setSensorSelection(seriesItem.childs[i], localSavedItem.childs[j]);
                        }
                    }
                }
            }
...
<<<<<<< HEAD
        ...
        if (this.selectedControllerSeries.tag == 'MVEC'
        && (item.tag == 'LD' || item.tag == 'PS')) {
           list.forEach((listItem) => {
               if (listItem.id != item.id) {
                   listItem.isChecked = false;
       ...
       if (this.selectedControllerSeries.tag == 'XS'
        && (item.tag == 'LD' || item.tag == 'PS')) {
           list.forEach((listItem) => {
               if (listItem.id != item.id) {
                   listItem.isChecked = false;
...
=======
        // if (item.tag == 'SI-1' || item.tag == 'AI - 1' || item.tag == 'AI - 2' || item.tag == 'AI - 3' || item.tag == 'AO - 1' || item.tag == 'AO - 2' || item.tag == 'AIAO') {
>>>>>>> 309b392c344eeefc42edd5eeea765ffec4ade513
        //     list.forEach((listItem) => {
        //         if (listItem.id != item.id) {
        //             listItem.isChecked = false;
        ...
        if ((item.tag == 'AN-PTSA-SEN' && item.childs.length > 0) ||
    ...
    aegisValidation(item: any) {
        ...
        if (this.selectedControllerSeries.tag == 'PA-2' ) {
            ...
            if ((item.tag == 'ORP-SEN' && item.isChecked) ||
            ...
            if ((item.tag == 'LD' && item.isChecked) || (item.tag == 'PS' && item.isChecked)) {
                this.analogItems = [];
            ...
            if ((item.tag == 'STST-01' && item.isChecked) ||
    ...
    aegisAdditionalioOptionValidation(item?: any) {
        ...
        for (let k = 0; k < this.seriesList.length; k++) {
            if (this.seriesList[k].tag == 'AG-AI/OO' && this.seriesList[k].childs && this.seriesList[k].childs.length) {
                for (let m = 0; m < this.seriesList[k].childs.length; m++) {
                    // If corrosion sensor options selected slot #1 ( D1 = active, H1 & I1 = inactive)
                    if (this.corrosionItems.length >= 2) {
                        this.seriesList[k].isChecked = true;
                        if (this.seriesList[k].childs[m].tag == 'Sl-01' && this.seriesList[k].childs[m].childs && this.seriesList[k].childs[m].childs.length > 0) {
                            this.seriesList[k].childs[m].isChecked = true;
                            for (let n = 0; n < this.seriesList[k].childs[m].childs.length; n++) {
                        ...
                        if (item) {
                            if (item.tag == 'STST-01' || item.tag == 'Admiralty' || item.tag == 'CUP-01' || item.tag == 'CS01' || item.tag == 'DC') {
                    ...
                    if (this.prominentItems.length > 0) {
                        this.seriesList[k].isChecked = true;
                        if (this.seriesList[k].childs[m].tag == 'Sl-02' && this.seriesList[k].childs[m].childs && this.seriesList[k].childs[m].childs.length > 0) {
                            this.seriesList[k].childs[m].isChecked = true;
                            for (let n = 0; n < this.seriesList[k].childs[m].childs.length; n++) {
                    ...
                    if (this.analogItems.length > 0) {
                        this.seriesList[k].isChecked = true;
                        if (this.seriesList[k].childs[m].tag == 'Sl-03' && this.seriesList[k].childs[m].childs && this.seriesList[k].childs[m].childs.length > 0) {
                            this.seriesList[k].childs[m].isChecked = true;
                            for (let n = 0; n < this.seriesList[k].childs[m].childs.length; n++) {
...
>>>>>>> 309b392c344eeefc42edd5eeea765ffec4ade513
    ...
    onMobileViewItemSelected(item, list) {
        ...
        if (this.selectedControllerSeries.tag == 'PA-2' && (item.tag == 'LD' || item.tag == 'PS')) {
            list.forEach((listItem) => {
                if (listItem.id != item.id) {
                    listItem.isChecked = false;
                }
        ...
        if (this.selectedControllerSeries.tag == 'MVEC' && (item.tag == 'LD' || item.tag == 'PS')) {
            list.forEach((listItem) => {
                if (listItem.id != item.id) {
                    listItem.isChecked = false;
                }
        ...
        if (this.selectedControllerSeries.tag == 'XS' && (item.tag == 'LD' || item.tag == 'PS')) {
            list.forEach((listItem) => {
                if (listItem.id != item.id) {
                    listItem.isChecked = false;
                }
    ...
    sensorSelectionValidCheck = () => {
        ...
        if (seriesSelectionStatus.isValid) {
            ...
            for (let i = 0; i < selectedSensorList.length; i++) {
                if (this.selectedControllerSeries.tag == 'WCT600P' || this.selectedControllerSeries.tag == 'WCT600H') {
                ...
                } else if (this.selectedControllerSeries.tag == 'XS') {  // makes no sense please test 
                    this.isValid = false;
                    sensorOption = true;
                }
            ...
            if (sensorOption && this.selectedControllerSeries.tag != 'PA-2' && this.selectedControllerSeries.tag != 'XS' && this.selectedControllerSeries.tag != 'MVEC') {

                // form only selected and checked item array
                this.copyOfSelectedSensorsList = JSON.parse(JSON.stringify(selectedSensorList));
                ...
                if (this.validSensorArrCheck && this.validSensorArrCheck.length > 0) {
                    let msg;
                    msg = 'Walchem sensors selection range : (minimum '+ this.WalchemMinLimit +' and maximum '+ this.WalchemMaxLimit +')'+
                            ' Analog sensors selection range : (minimum '+ this.AnalogMinLimit +' and maximum '+ this.AnalogMaxLimit +')'+
                            ' Sensor option selection range : (minimum '+ this.sensorOptionMinLimit +' and maximum '+ this.sensorOptionMaxLimit +')'+
                    ...
                    this.isValid = true;
                } else if (this.validAdditionalIOSensorArrCheck && this.validAdditionalIOSensorArrCheck.length > 0) {
                    let msg;
                    msg = 'Additional I/O options selection range: (minimum '+ this.additionalIOMinLimit +' and maximum '+ this.additionalIOMaxLimit+')';
                    this.commonService.showAlert("Alert", msg, "OK", () => {
                        this.limitValidation(this.copyOfSelectedSensorsList);
                    });
                    this.isValid = true;
                } else if (this.validWifiOptionsArrCheck && this.validWifiOptionsArrCheck.length > 0) {
                    let msg;
                    msg = 'Wifi options selection range: (minimum '+ this.wifiOptionMinLimit +' and maximum '+ this.wifiOptionMaxLimit+')';
                    this.commonService.showAlert("Alert", msg, "OK", () => {
                        this.limitValidation(this.copyOfSelectedSensorsList);
                    });
                    this.isValid = true;
            ...
            if (!this.isValid) {
                this.commonService.storeDataInCreateQuoteVo('selectedSensorList', selectedSensorList);
            }
            ...
            if (seriesSelectionStatus.reason == 'No series Item selection') {
                ...
                this.commonService.showAlert("Info", 'Please select at least one controller sensor & I/O.', "OK", () => {
                ...
                for (let k = 0; k < this.invalidSeriesSelectionArray.length; k++) {
                    if (k == 0) {
                        ...
                        invalidSeries = invalidSeries + ', ' + this.invalidSeriesSelectionArray[k].seriesName;
                    }
                }
                let messageText: string = 'You have incomplete Controller Sensor & I/O selection for ' + '"' + invalidSeries + '"' + ', Do you want to discard the selection?';
            ...
            for (let i = 0; i < this.seriesList.length; i++) {
                if (this.invalidSeriesSelectionArray.length) {
                    for (let j = 0; j < this.invalidSeriesSelectionArray.length; j++) {
                        if (this.seriesList[i].id == this.invalidSeriesSelectionArray[j].seriesId) {
                        ...
                        if (this.selectedControllerSeries.tag == 'PA-2'  && this.invalidSeriesSelectionArray[j].seriesName == 'Sensor Options') {
                          
                                    if (this.prominentItems.length > 0 || this.corrosionItems.length > 0 || this.analogItems.length > 0) {
                                        for (let k = 0; k < this.seriesList.length; k++) {
                                            if (this.seriesList[k].tag == 'AG-AI/OO' && this.seriesList[k].isChecked && this.seriesList[k].childs && this.seriesList[k].childs.length) {
                                                this.seriesList[k].isChecked = false;
                                                this.analogItems = [];
                                                this.prominentItems = [];
                                                this.corrosionItems = [];
                                                for (let m = 0; m < this.seriesList[k].childs.length; m++) {
                                                    if (this.seriesList[k].childs[m].isChecked && this.seriesList[k].childs[m].childs && this.seriesList[k].childs[m].childs.length > 0) {
                                                        this.seriesList[k].childs[m].isChecked = false;
                                                        for (let n = 0; n < this.seriesList[k].childs[m].childs.length; n++) {
                                                            if (this.seriesList[k].childs[m].childs[n].isChecked) {
                                                                this.seriesList[k].childs[m].childs[n].isChecked = false;
                                                            } else if (this.seriesList[k].childs[m].childs[n].disable) {
                                                                this.seriesList[k].childs[m].childs[n].disable = false;
                                                            }
                                                        }
                                                    }
                                ...
                                // if invalid selection in sensor option and additional I/O options are selected then remove selection.
                        if (this.selectedControllerSeries.tag == 'MVEC'  && this.invalidSeriesSelectionArray[j].seriesName == 'Sensor Options') {
                           ...
                           //if invalid selection in sensor option and additional I/O options are selected then remove selection.
                           if (this.selectedControllerSeries.tag == 'XS'  && this.invalidSeriesSelectionArray[j].seriesName == 'Sensor Options') {
        ...
        if (item && item.childs && item.childs.length > 0) {
            for (let j = 0; j < item.childs.length; j++) {
                ...
                if (item.childs[j].tag == "AN-SEN") {
                    ...
                    if (item.childs[j].count >= this.AnalogMinLimit && item.childs[j].count <= this.AnalogMaxLimit) {
                        item.childs[j].isValidChildSelection = true;
                    } else {
                        item.childs[j].isValidChildSelection = false;
                    }
<<<<<<< HEAD
                } else if (item.childs[j].tag == "WAL-SEN" || item.childs[j].tag == "WAL-CS") {
=======
                } else if (item.childs[j].tag == "WAL-SEN") {
>>>>>>> 309b392c344eeefc42edd5eeea765ffec4ade513
                    this.WalchemMaxLimit = item.childs[j].max_childs;
                    ...
                    this.childRecursion(item.childs[j]);
                    item.childs[j].count = this.count;
                    if (item.childs[j].count >= this.WalchemMinLimit && item.childs[j].count <= this.WalchemMaxLimit) {
                        item.childs[j].isValidChildSelection = true;
                    } else {
                        item.childs[j].isValidChildSelection = false;
                    }
...
>>>>>>> 309b392c344eeefc42edd5eeea765ffec4ade513
                    if (parentCount >= this.sensorOptionMinLimit && parentCount <= this.sensorOptionMaxLimit) {
                        this.isSensorSelectionInLimitValid = false;
                    } else {
   ...
   additionalIOValidation(item) {
        ...
        if (item && item.childs && item.childs.length > 0) {
            for (let j = 0; j < item.childs.length; j++) {
                ...
                this.childRecursion(item.childs[j]);
                item.childs[j].count = this.count;
                if (item.childs[j].count >= item.min_childs && item.childs[j].count <= item.max_childs) {
                    item.childs[j].isValidChildSelection = true;
                } else {
                    item.childs[j].isValidChildSelection = false;
                }
                ...
                if (item.childs[j].isValidChildSelection) {
                    ...
                    if (parentItemCount >= item.min_childs && parentItemCount <= item.max_childs) {
